...
export const signup: FuncM = (model: string) => {
  return catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!newUser) {
      return next(new AppError('User was not created', 401));
    }
    ...
    const url = `${req.protocol}://${req.get(
      'host'
    )}/api/v1/${routeUserType}/confirmation/${emailToken}/${newUser.name}`;
  ...
  return catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (model === 'User') {
      ...
      if (!currentUser) {
        return next(
          new AppError(
            'If you have already signed up, please confirm your email! You have been here before!',
            401
          )
        );
      }
...
export const emailConfirm: FuncM = model => {
  return catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!user) {
      return next(
        new AppError(
          'Sorry but we could not confirm your email, please try again!',
          400
        )
      );
    }
    ...
    if (model === 'CoffeeProvider') {
      user = await CoffeeProvider.findOneAndUpdate(
        { _id: user._id },
        { $set: { emailConfirm: true } }
      );
    }
...
export const resendEmailConfirmationToken: FuncM = model => {
  return catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const url = `${req.protocol}://${req.get(
      'host'
    )}/api/v1/${routeUserType}/confirmation/${token}/${user.name}`;
